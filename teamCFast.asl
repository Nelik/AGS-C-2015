...
+!heuristic(NodeX, NodeY, Return) <-
	Return = 0. // new goal will be computed -> goal is an obstacle
...
+!aStar(Sx, Sy, Gx, Gy) <-
	...
	+continue(1);
	while(continue(1) & aStarGoal(TarX, TarY)){
		!getLowestRank;
		-continue(_);
	...
	if (aStarGoal(TarX, TarY))
	{
		...
		while(continue(1))	
		{
			...
			if(closedSet(Ax, Ay, Px, Py, _, _)){
				-closedSet(Ax, Ay, Px, Py, _, _);
				+path(Px,Py);
				-actualX(_);
				-actualY(_);
				+actualX(Px);
